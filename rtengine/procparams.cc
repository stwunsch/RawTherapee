        ...
        for (size_t i = 0; i < locallab.spots.size(); ++i) {
            if (!pedited || i < pedited->locallab.spots.size()) {
                ...
                if ((!pedited || spot_edited->visiblur) && spot.visiblur) {
                    ...
                    saveToKeyfile(!pedited || spot_edited->limden, "Locallab", "limden_" + index_str, spot.limden, keyFile);
                    ...
                    saveToKeyfile(!pedited || spot_edited->usemask, "Locallab", "Usemask_" + index_str, spot.usemask, keyFile);
                    ...
                    saveToKeyfile(!pedited || spot_edited->levelthr, "Locallab", "Levelthr_" + index_str, spot.levelthr.toVector(), keyFile);
                }
                // Tone Mapping
    ...
    try {
        ...
        if (keyFile.has_group("Locallab")) {
            ...
            while (assignFromKeyfile(keyFile, "Locallab", "Name_" + std::to_string(i), pedited, ppName, peName)) {
                ...
                assignFromKeyfile(keyFile, "Locallab", "limden_" + index_str, pedited, spot.limden, spotEdited.limden);
                ...
                assignFromKeyfile(keyFile, "Locallab", "Usemask_" + index_str, pedited, spot.usemask, spotEdited.usemask);
                ...
                if (keyFile.has_key("Locallab", "Levelthr_" + index_str)) {
                    const std::vector<double> thresh = keyFile.get_double_list("Locallab", "Levelthr_" + index_str);

                    if (thresh.size() >= 2) {
                        spot.levelthr.setValues(thresh[0], thresh[1]);
                    }

                    spotEdited.levelthr = true;
                }
